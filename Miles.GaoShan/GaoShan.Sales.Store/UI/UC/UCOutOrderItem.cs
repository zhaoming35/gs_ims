...
{
    public partial class UCOutOrderItem : GaoShan.Sales.UI.UC.UCBase
    {
        Logic.StoreManager manager = new Logic.StoreManager();
        Logic.MatchTbManager matchTbManager = new Logic.MatchTbManager();

        public UCOutOrderItem()
        {
            InitializeComponent();
        }
        ...
        //根据配套部的项目，查询
        Dictionary<string, DbDecValue> dicMatchTbItems;
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            ...
            Model.Info_store_out_item item = mDataGridItem.GetCurrentEntity() as Model.Info_store_out_item;

            string msg = string.Format("确定删除:{0}?", item.Mtl_Name);
            if (Miles.Framework.UI.Function.Dialog.ShowQuestionYesNo(msg))
            {
                mDataGridItem.DeleteCurrentItem<Model.Info_store_out_item>();
            }
        }
        ...
        private void BtnAdd_Click(object sender, EventArgs e)
        {
            ...
            fm.SelectValue += (obj) =>
            {
                ...
                foreach(var v in mDataGridItem.GetDataSource<Model.Info_store_out_item>())
                {
                ...
                Model.Info_store_out_item item = Miles.Coro.Common.Utils.CopyToOther<Info_mtl_item, Model.Info_store_out_item>(mtl);
                item.Plan_OutQty = dicMatchTbItems.ContainsKey(item.Mtl_Code) ? dicMatchTbItems[item.Mtl_Code].Value:0;
                mDataGridItem.AddItem<Model.Info_store_out_item>(item);
            };
            fm.ShowDialog();
        }
